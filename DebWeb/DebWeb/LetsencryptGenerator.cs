namespace DebWeb
{
    public static class LetsencryptGenerator
    {
        ...
        {
            $"sudo {systemSettings.LetsencryptPath}/certbot-auto certonly -n --agree-tos --email {appSettings.UserEmail} --rsa-key-size 4096 --webroot --webroot-path {appSettings.ProjectPath} -d {string.Join(" -d ", appSettings.Dns)}".Bash();
        }
